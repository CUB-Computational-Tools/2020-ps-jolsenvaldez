...
tibble(
  text = c("Sandy Mars", "Icy Mars"), # Entering row names
  ...
  TempK = (((1-albedo)*So)/(4*SBc))^(1/4), #calculate Temperature in Kelvin using the terms we defined in our data frame
  TempC = (TempK - 273.15) #calculate the Temperature in degrees Celsius
)
Mars_albedos$TempC_diff = lead(Mars_albedos$TempC) - Mars_albedos$TempC #calculate the difference in the TempC between Sandy and Icy Mars and then add a column
...
plot_Mars_albedo <- (Mars_albedos %>%
       ggplot() + 
       aes(x = albedo, y=TempC) + 
       scale_x_continuous(limits = c(0.15,0.70), breaks=seq (0.15,0.70,0.05)) +
       geom_point() +
       labs (title = "Temperature of Mars for two different albedos", x = "Mars albedo", y= "Temperature (degrees Celsius)") +
       geom_line()) +
       theme_bw()
plot_Mars_albedo
```
...
2) Solve for the Temperature of the earth using its modern albedo of 0.37. Hint: you can do this with a variable and list like we did last week. 
...
3) Follow the steps below to practice using coding language to investigate the temperature of the earth given varying albedo values: 
   ...
   b) add an emissivity column (based on albedo) and a calculated temperature column for the data frame. 
...
Earth_albedos$TempC_diff = lead(Mars_albedos$TempC) - Mars_albedos$TempC #calculate the difference in the TempC between Sandy and Icy Mars and then add a column
Earth_albedos #we call our named data frame and it displays below!
```
 
_________________________________________________________

4) After completing question 3, what do you think about the temperatures that you calculated? Do they seem too hot or too cold? If they weren't what you were expecting given the temerature of Earth today, what could be missing from the model that we considered in calculating the earth's temperature? 
